...
import {
  getCafeRankTrackingDetail,
  stopCafeRankTracking,
} from "@/Features/apis/admin/cafeRank"
import type { CafeHistoryItem } from "@/Features/apis/admin/cafeRank"
...
export function AdminCafeRankTrackingDetail({ trackingId }: { trackingId: number }) {
  const router      = useRouter()
  const queryClient = useQueryClient()

  const { data, isLoading, isError } = useQuery({
    queryKey: ["admin", "cafe-rank", "tracking", trackingId],
    queryFn: () => getCafeRankTrackingDetail(trackingId),
    staleTime: 30 * 1000,
  })

  const stopMutation = useMutation({
    mutationFn: () => stopCafeRankTracking(trackingId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin", "cafe-rank", "tracking", trackingId] })
      queryClient.invalidateQueries({ queryKey: ["admin", "cafe-rank", "tracking"] })
    },
  })
    ...
    <div className="space-y-4 max-w-3xl">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold" style={{ color: "var(--th-text-1)" }}>카페 순위 추적 상세</h1>
        ...
        <button
          onClick={() => router.push("/admin/cafe-rank")}
        ...
        <>
          ...
          <div className="space-y-3">
            ...
            {(() => {
              ...
              const grouped = new Map<number, CafeHistoryItem[]>()
