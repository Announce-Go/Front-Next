...
export function AdminBlogRankTrackingDetail({ trackingId }: { trackingId: number }) {
  const router      = useRouter()
  const queryClient = useQueryClient()

  const { data, isLoading, isError } = useQuery({
    queryKey: ["admin", "blog-rank", "tracking", trackingId],
    queryFn: () => getBlogRankTrackingDetail(trackingId),
    staleTime: 30 * 1000,
  })

  const stopMutation = useMutation({
    mutationFn: () => stopBlogRankTracking(trackingId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin", "blog-rank", "tracking", trackingId] })
      queryClient.invalidateQueries({ queryKey: ["admin", "blog-rank", "tracking"] })
    },
  })
  ...
  return (
    <div className="space-y-4 max-w-3xl">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => router.push("/admin/blog-rank")}
