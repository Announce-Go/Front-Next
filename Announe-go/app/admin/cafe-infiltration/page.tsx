...
import {
  getCafeInfiltrationMonthlyList,
  createCafeInfiltration,
  updateCafeInfiltration,
  deleteCafeInfiltration,
} from "@/Features/apis/admin/cafeInfiltration"
import type { CafeInfiltrationItem } from "@/Features/apis/admin/cafeInfiltration"
...
type FormState =
  | null
  | { mode: "create" }
  | { mode: "edit"; id: number; title: string; content: string; cafe_name: string; url: string; advertiser_id: number }
...
function CafeInfiltrationContent() {
  ...
  const { data: monthlyData, isLoading: monthlyLoading } = useQuery({
    queryKey: ["admin", "cafe-infiltration", year, month],
    queryFn: () => getCafeInfiltrationMonthlyList(year, month),
    enabled: !!year && !!month,
    staleTime: 30 * 1000,
  })
  ...
  monthlyData?.items?.forEach((a) => {
    countMap[a.infiltration_date] = (countMap[a.infiltration_date] ?? 0) + 1
  })

  /* 선택 날짜 목록 */
  const selectedItems: CafeInfiltrationItem[] = selectedDate
    ? (monthlyData?.items ?? []).filter((a) => a.infiltration_date === selectedDate)
    : []
  ...
  const openEdit = (item: CafeInfiltrationItem) => {
    setFormTitle(item.title)
    setFormContent(item.content)
    setFormCafeName(item.cafe_name)
    setFormUrl(item.url)
    setFormAdvertiserId(item.advertiser_id)
    setFormState({ mode: "edit", id: item.id, title: item.title, content: item.content, cafe_name: item.cafe_name, url: item.url, advertiser_id: item.advertiser_id })
  }
  ...
  /* mutations */
  const invalidate = () => {
    queryClient.invalidateQueries({ queryKey: ["admin", "cafe-infiltration", year, month] })
  }

  const createMutation = useMutation({
    mutationFn: () => createCafeInfiltration({
      advertiser_id:     Number(formAdvertiserId),
      infiltration_date: selectedDate!,
      title:             formTitle.trim(),
      content:           formContent.trim(),
      cafe_name:         formCafeName.trim(),
      url:               formUrl.trim(),
    }),
  ...
  const updateMutation = useMutation({
    mutationFn: () => {
      ...
      return updateCafeInfiltration(s.id, {
        advertiser_id: Number(formAdvertiserId),
        title:         formTitle.trim(),
        content:       formContent.trim(),
        cafe_name:     formCafeName.trim(),
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => deleteCafeInfiltration(id),
  ...
  const handleDelete = (id: number) => {
    if (!confirm("카페 침투를 삭제하시겠어요?")) return
  ...
  return (
    <div className="space-y-5">
      {/* 헤더 */}
      <div>
        <h1 className="text-xl font-bold" style={{ color: "var(--th-text-1)" }}>카페 침투 관리</h1>
        <p className="text-xs mt-0.5" style={{ color: "var(--th-text-3)" }}>
          날짜를 클릭하여 카페 침투를 조회하고 등록·수정·삭제할 수 있습니다.
        </p>
      </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold" style={{ color: "var(--th-text-1)" }}>
              {formatDisplayDate(selectedDate)}의 카페 침투
            ...
            >
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold" style={{ color: "var(--th-text-1)" }}>
                  {formatDisplayDate(selectedDate)} — 카페 침투 {formState.mode === "create" ? "등록" : "수정"}
                </h3>
                <button
                  onClick={closeForm}
              ...
              <div className="flex items-center justify-end gap-2 pt-1">
                ...
                <button
                  onClick={handleSave}
                  disabled={!formTitle.trim() || !formContent.trim() || !formAdvertiserId || isPending}
