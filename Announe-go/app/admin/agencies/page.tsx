...
function AgenciesContent() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["admin", "agencies", { search: urlSearch, page }],
  ...
  return (
    <div className="space-y-5">
      {/* 헤더 */}
      <div>
        <h1 className="text-xl font-bold" style={{ color: "var(--th-text-1)" }}>업체 관리</h1>
        <p className="text-xs mt-0.5" style={{ color: "var(--th-text-3)" }}>
          승인된 업체 목록을 확인하고 상세 정보를 조회할 수 있어요.
        </p>
      </div>
      ...
      >
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr style={HEAD_ST}>
              <th className="py-3 px-4 text-left font-medium border-b" style={{ borderColor: "var(--th-row-border)" }}>업체명</th>
              ...
              <th className="py-3 px-4 text-left font-medium border-b hidden md:table-cell" style={{ borderColor: "var(--th-row-border)" }}>이용 카테고리</th>
          ...
          <tbody>
            {isLoading ? (
              ...
              items.map((item) => (
                <tr
                  key={item.id}
                  onClick={() => router.push(`/admin/agencies/${item.id}`)}
