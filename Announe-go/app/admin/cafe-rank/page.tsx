...
function CafeRankContent() {
  ...
  const realtimeMutation = useMutation({
    mutationFn: () => getCafeRankRealtime({ keyword: rtKeyword.trim(), url: rtUrl.trim() }),
  })

  const { data, isLoading } = useQuery({
    queryKey: ["admin", "cafe-rank", "tracking", { status: urlStatus, search: urlSearch, page }],
    queryFn: () => getCafeRankTrackingList({
  ...
  return (
    <div className="space-y-6">
      {/* 헤더 */}
      <div>
        <h1 className="text-xl font-bold" style={{ color: "var(--th-text-1)" }}>카페 순위 관리</h1>
        ...
        <div className="flex flex-wrap items-end gap-3">
          <div className="flex flex-col gap-1 flex-1 min-w-[160px] max-w-xs">
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter" && rtKeyword.trim() && rtUrl.trim()) realtimeMutation.mutate()
              }}
              ...
              {isLoading ? (
                ...
                items.map((item) => {
                  ...
                  return (
                    <tr
                      key={item.id}
                      onClick={() => router.push(`/admin/cafe-rank/tracking/${item.id}`)}
