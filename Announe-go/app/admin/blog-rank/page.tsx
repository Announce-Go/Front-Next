...
function BlogRankContent() {
  ...
  const realtimeMutation = useMutation({
    mutationFn: () => getBlogRankRealtime({ keyword: rtKeyword.trim(), url: rtUrl.trim() }),
  })

  const { data, isLoading } = useQuery({
    queryKey: ["admin", "blog-rank", "tracking", { status: urlStatus, search: urlSearch, page }],
    queryFn: () => getBlogRankTrackingList({
  ...
  return (
    <div className="space-y-6">
      {/* 헤더 */}
      <div>
        <h1 className="text-xl font-bold" style={{ color: "var(--th-text-1)" }}>블로그 순위 관리</h1>
              ...
              {isLoading ? (
                ...
                items.map((item) => {
                  ...
                  return (
                    <tr
                      key={item.id}
                      onClick={() => router.push(`/admin/blog-rank/tracking/${item.id}`)}
