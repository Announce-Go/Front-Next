...
export default async function AgencyPlaceRankPage({
  searchParams,
}: {
    ...
    >
      ...
      <div className="relative mb-8">
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold text-white">플레이스 순위 관리</h1>
        </div>
        <p className="text-sm ml-[52px]" style={{ color: "#94a3b8" }}>
          담당 광고주의 플레이스 순위를 조회하고 추적 키워드를 등록해요.
        </p>
      </div>

      <div className="relative space-y-6">
        <RealtimeRankCard />
        <PlaceRankTrackingCard searchParams={params} />
      </div>
    </div>
  )
}
