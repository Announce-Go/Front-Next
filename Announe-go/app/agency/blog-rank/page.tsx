...
export default async function AgencyBlogRankPage({
  searchParams,
}: {
    ...
    >
      ...
      <div className="relative mb-8">
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold text-white">블로그 글 순위 관리</h1>
        </div>
        <p className="text-sm ml-[52px]" style={{ color: "#94a3b8" }}>
          담당 광고주의 블로그 게시글 순위를 조회하고 추적 키워드를 등록해요.
        </p>
      </div>

      <div className="relative space-y-6">
        <BlogRealtimeRankCard />
        <BlogRankTrackingCard searchParams={params} />
      </div>
    </div>
  )
}
