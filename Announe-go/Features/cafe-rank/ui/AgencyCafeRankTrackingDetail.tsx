...
export function AgencyCafeRankTrackingDetail({ trackingId }: { trackingId: number }) {
  const router = useRouter()

  const { data, isLoading, isError } = useQuery({
    queryKey: ["agency", "cafe-rank", "tracking", trackingId],
    queryFn: async () => {
      const res = await http.get<TrackingDetail>(`/api/v1/agency/cafe-rank/tracking/${trackingId}`)
      return res.data
    },
    staleTime: 30 * 1000,
  })
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">카페 글 순위 추적 상세</h1>
        ...
        <button
          onClick={() => router.push("/agency/cafe-rank")}
