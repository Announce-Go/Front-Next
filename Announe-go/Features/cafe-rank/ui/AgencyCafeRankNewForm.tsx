...
export function AgencyCafeRankNewForm({ advertisers }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    if (!url.trim()) { toast.warning("카페 게시글 URL을 입력해주세요."); return }
    if (!selectedId) { toast.warning("광고주를 선택해주세요."); return }

    setIsSubmitting(true)
    try {
      await http.post("/api/v1/agency/cafe-rank/tracking", {
        keyword: keyword.trim(),
        url: url.trim(),
        advertiser_id: selectedId,
      })
      toast.success("추적 등록이 완료됐어요.")
      router.push("/agency/cafe-rank")
    } catch (e) {
      console.error(e)
  ...
  return (
    ...
    >
      ...
      <div className="space-y-1.5">
        ...
        <input
          type="text"
          placeholder="https://cafe.naver.com/article/..."
          value={url}
