...
import { BlogRankTrackingControls } from "@/Features/blog-rank/ui/BlogRankTrackingControls"
...
export async function BlogRankTrackingCard(props: { searchParams?: SearchParams }) {
  ...
  try {
    data = await serverFetch<TrackingListResponse>(`/api/v1/agency/blog-rank/tracking${qs}`, {
      cache: "no-store",
    })
  } catch (e) {
  ...
  const prevHref = (() => {
    ...
    return `/agency/blog-rank?${p.toString()}`
  })()
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
                {["키워드", "URL", "광고주", "회차", "진행률", "최근 체크", "최근순위", "상태", ""].map((h) => (
                  <th
                    key={h}
            ...
            <tbody>
              {items.map((it) => {
                ...
                const progressCount = typeof it.progress_count === "number" ? it.progress_count : null
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex flex-col gap-1 min-w-[80px]">
                        <span className="text-xs tabular-nums" style={{ color: "#94a3b8" }}>
                          {progressCount !== null ? `${progressCount} / 25회` : "-"}
                        </span>
                        {progressCount !== null && (
                          <div
                            className="h-1 rounded-full overflow-hidden"
                            style={{ background: "rgba(255,255,255,0.08)", width: "80px" }}
                          >
                            <div
                              className="h-full rounded-full"
                              style={{
                                width: `${Math.min(100, (progressCount / 25) * 100)}%`,
                    ...
                    <td className="px-4 py-3.5">
                      <Link
                        href={`/agency/blog-rank/tracking/${it.id}`}
